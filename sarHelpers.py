...
def preprocessSarFile(sarDownloadFilePath, geopandasDataFilePath, geoDataIndex, dstPath):
    '''
    ...
    start = time()
    if isinstance(sarDownloadFilePath, list):
        out_1 = filenamePrefix + '_1.tif'
        out_2 = filenamePrefix + '_2.tif'
        subprocess.call(["python", "getGeoTiffImage.py", geopandasDataFilePath, str(geoDataIndex), out_1, 
                        sarDownloadFilePath[0]])
        subprocess.call(["python", "getGeoTiffImage.py", geopandasDataFilePath, str(geoDataIndex), out_2, 
                        sarDownloadFilePath[1]])
        rawImg = filenamePrefix + '_merged.tif'
        rawImg = mergeGeoTiff(out_1, out_2, rawImg)
    else:
        subprocess.call(["python", "getGeoTiffImage.py", geopandasDataFilePath, str(geoDataIndex), dstPath, 
                        sarDownloadFilePath])
        rawImg = filenamePrefix + '_raw1.tif'

    print('Get geotiff image in %0.3fs' % (time() - start))
    waterBodyFilename = getWaterBodyFromFileAndSave(rawImg, filenamePrefix + '_waterBody.tif')
    return resize(sarImgDir=waterBodyFilename, 
                  imgDst=dstPath,
                  maxSize=400)
